...
public class WordLadderII {
	 ...
	 * 
	 * @intuition
	 * 		The critical part in this exercise was to create a good graph.
	 * 		Else things could go crazy in terms of time complexity.
	 * 
	 * 		This is not the cleanest solution, I need to redo this.
	 * 
	 * 		Backtracking with memoization. Where Graph was created with bfs and the solution itselft was done with dfs.
	 * 
	 * 		The goal was to find the target word and from there add to a memo the answer.
	 * 			
	 * 			the target node will have a memo with its own word.
	 * 
	 * 			the nodes in the level behind will have [currentNode] + [last node]
	 * 
	 * 			and continues till we reach the first node where its garanteed to have the shortest paths
	 * 
	 * 		Then for the other nodes save always the smallest of the paths
	 * 
	 * @comments
	 * 		I had a lot of time limit exceeded before comming with the right approach to solve the problem
	 * 
	 * @optimizations
	 * 		in the for loop I Could use a char right away and increase the char for (int i = a; i < z; c++)
	 * 		I could cut iterations in the graph creation where the characters are equal to the original word
	 * @score
	 * 		Runtime: 181 ms, faster than 47.18% of Java online submissions for Word Ladder II.
			Memory Usage: 115.2 MB, less than 5.02% of Java online submissions for Word Ladder II.
	 * 
	 * 		Runtime: 96 ms, faster than 72.35% of Java online submissions for Word Ladder II.
			Memory Usage: 44.8 MB, less than 63.41% of Java online submissions for Word Ladder II.
	 * 
	 * @fail   
	 * 		1) I thought that queue was also a list
	 * 		2) forgot to only memo if not null
	 * 		3) I was not updating object correctly, I've screwed some references along the way
			4) comparison with min value was wrong, bad calculation to have the size of the queue
			5) adding the wrong variable to the queue
			6) duplicates not took into account
			7) Time limit exceeded

			1) creating graph was bad
			2) was making elements as visited too late
			3) I was enqueing elements more than once
	...
	public List<List<String>> findLadders(String beginWord, String endWord, List<String> words) {
		...
		//build graph
		bfs(beginWord, words, graph);

		if(graph.get(beginWord) == null)
			return new ArrayList<List<String>>();


		List<List<String>> withDuplicates = dfs(beginWord, endWord, graph, memo, visited, 1);


		Set<List<String>> unique = new HashSet<>();
		List<List<String>> ans = new ArrayList<List<String>>();
		for (List<String> l : withDuplicates) //O(N) N equals to unique replies
		{
	 ...
	 * 		O(V * E * A * W)
	 * 		A answer (mas value of a is wordlist size for each list entry)
	 * 		W word size
	 * 
	 * @param node
	 * @param endWord
	...
	(String node, String endWord, HashMap<String, List<String>> graph, HashMap<String, List<List<String>>> memo, Set<String> visited, int size)
	{
		if (size > chainLength)
			return null;


		if (memo.getOrDefault(node, new ArrayList<List<String>>()).size() != 0)
			return memo.get(node);

		int min = Integer.MAX_VALUE;

		//Base case
		if (node.equals(endWord) && size <= min)
		{
			List<List<String>> ans = new ArrayList<List<String>>();
			ans.add(new LinkedList<String>());
			ans.get(0).add(endWord);
			memo.put(endWord,ans);

			return memo.get(endWord); 
		}


		visited.add(node);
		List<List<String>> nodeLists = new ArrayList<List<String>>();
		List<List<String>> childrensList = new ArrayList<List<String>>();
		List<String> neighbors = graph.getOrDefault(node, new ArrayList<String>());
		LinkedList<String> q;
		for (String nei: neighbors) //O(E)
		{
			if (!visited.contains(nei))
			{

				childrensList =  dfs(nei, endWord, graph, memo, visited, size + 1);

				if (childrensList != null)
					for (List<String> l : childrensList)
					{
						q = (LinkedList<String>) l;

						if (size + q.size() == min)
						{	
							q = new LinkedList<String>(l); //O(W)
							q.addFirst(node);
							nodeLists.add(q);
						}
						else if (size + q.size() < min)
						{
							q = new LinkedList<String>(l);
							chainLength = size + q.size();
							nodeLists.clear(); //might be O(N)
							q.addFirst(node);
							//nodeLists.add(l);
							nodeLists.add(q);
							min = chainLength;
		...
		//memo
		if (nodeLists.size() > 0)
			memo.put(node, nodeLists);

		return nodeLists;
	}


	private void bfs(String beginWord, List<String> words, HashMap<String, List<String>> graph)
	{
		...
		Set<String> curNextWords;
		Set<String> visited = new HashSet<>();

		Node cur;
		nodeLevel.put(beginWord, 0);

		StringBuilder tmp;
		char old;
		String candidate;

		while(!q.isEmpty()) //O(N) N is words
		{
			cur = q.poll();
			if(visited.contains(cur.w))
				continue;
			tmp = new StringBuilder(cur.w);
			visited.add(cur.w);
			curNextWords = new HashSet<>();
			for (int i = 0; i < tmp.length(); i++) //O(W) W word size
			{
				old = tmp.charAt(i);
				...
				for (int j = 0; j < 26; j++)
				{
					...
					candidate = tmp.toString();
					if (wordSet.contains(candidate) && 
							nodeLevel.getOrDefault(candidate, Integer.MAX_VALUE) > cur.lvl &&
							!curNextWords.contains(candidate))
					{
						graph.computeIfAbsent(cur.w, k -> new ArrayList<String>()).add(candidate);
						nodeLevel.put(candidate, cur.lvl + 1);
						q.add(new Node(candidate, cur.lvl + 1));
						curNextWords.add(candidate);
					}
				}
	...
	class Node{
		String w;//word
		int lvl;
		public Node(String w, int level)
		{
			this.w = w;
			lvl = level;
		}
...
/**
 * This solution intelligently uses bidirectional bfs to find the shortest path to the endword
 * 
 * using biDirectional bfs is great advantage because it creates a smaller graph, and therefore the search will be more efficient
 * 
 * bidirectional bfs do not do the whole graph traversal, and that's why it is so cool
 * 
 * after creating a very short graph, the author has the guarantee that all paths will be the shortest be cause we will have a worst multiple straightlines from the start node to the end node.
 * 
 * each straight line passes through precisely one node.
 * 
 * There is a lot of great things being done here
 * 
 * the search ends when the next set collection are contained in 
 * 
 * 
 * @author Nelson Costa
 *
 */
class WordLadderIIUnofficialSolution1 {
	...
	/**
	 * They remove the words from the set as they progress
	 * This is indeed a bi Directional Search
	 * Builds a forward graph (it is not reversed)
	 * 
	 * @param begin
	 * @param end
	 * @param set
	 ...
	 * @param reverse
	 */
	private void bfs(Set<String> begin, Set<String> end, Set<String> set, Map<String, List<String>> map, boolean reverse){
		boolean found = false;
		while (!found && !begin.isEmpty()){
			if(begin.size() > end.size()){
				bfs(end,begin,set, map, !reverse);
				return;
			}
			set.removeAll(begin);
			...
			for (String s: begin){
				...
				for(int i=0; i< n; i++){
					...
					for (char c = 'a'; c <= 'z'; c++){
						if(c == tmp) continue;
						crr[i] = c;
						String tmpstr = new String(crr);
						if(set.contains(tmpstr)){
							if(end.contains(tmpstr)) 
								found = true;
							else        
								next.add(tmpstr);

							String key = reverse ? tmpstr: s;
							String val = reverse ? s : tmpstr;
